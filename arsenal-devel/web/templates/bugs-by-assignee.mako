...
<%
    ...
    for bid in tasks:
        for t in tasks[bid]:
            assignee = 'unknown' if t['assignee'] == '' else t['assignee']

            if assignee not in bugs_by_assignee:
                bugs_by_assignee[assignee] = {}

            if bid not in bugs_by_assignee[assignee]:
                bugs_by_assignee[assignee][bid] = []

            if t['bug_target_name'] not in bugs_by_assignee[assignee][bid]:
                bugs_by_assignee[assignee][bid].append(t['bug_target_name'])

    assignee_report_order = []
    if 'unknown' in bugs_by_assignee:
        assignee_report_order.append('unknown') # We want unknown first
    for t in sorted(bugs_by_assignee):
        if t != 'unknown':
            assignee_report_order.append(t)
%>
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr" lang="en-US">
    ...
    <body class="bugbody">
        ...
        <div class="outermost">
            ...
            <div class="section">
                % for assignee in assignee_report_order:
                <div class="section-heading">${assignee} (${len(bugs_by_assignee[assignee].keys())})</div>

                    <% id = assignee.replace(' ', '_') %>
            ...
            <div>
                ...
                <table width="100%%" cellspacing="0" cellpadding="0">
                    ...
                    <tbody>
                        ...
                        <tr><td>Release    </td><td>The task to which the release is targeted.                           </td></tr>
                        ...
                        <tr><td>Last Update</td><td>Number of days ago that the last update to the bug occurred.        </td></tr>
                    </tbody>
                </table>
                <br />
            </div>
    ...
    <script type="text/javascript">
        ...
        var assignees_id_list = [];
        var assignees_name_list = [];
        % for assignee in assignee_report_order:
            <% id = assignee.replace(' ', '_') %>
            assignees_id_list.push("${id}");
            assignees_name_list.push("${assignee}");
        % endfor
            ...
            $.each(jd, function(bid, tasks) {
                $.each(tasks, function(index, task) {
                    ...
                    */
                    ...
                    s = "";
                    if (!fail) {
                        if (oddness[task.assignee]) {
                            s += "<tr class=\"odd\">";
                            oddness[task.assignee] = false;
                        } else {
                            s += "<tr class=\"even\">";
                            oddness[task.assignee] = true;
                        }
                        s += "<td><a href=\"http://launchpad.net/bugs/" + bid + "\">" + bid + "</a></td>";
                        s += "<td>" + task.bug.title + "</td>";
                        s += "<td>" + task.task_name + "</td>";
                        s += "<td>" + task.nominations + "</td>";
                        ...
                        s += "<td>" + task.bug.last_update + "</td>";
                        s += "</tr>";
                        tables[task.assignee] += s;
                    }
                });
            });

            $.each(tables, function(assignee, val) {
                id = assignee.replace(/ /g, '_');
                $("#" + id + " tbody").html(tables[assignee]);
                $("#" + id).trigger("update");
            });
            if (first_time) {
                first_time = false;
