...
<%
    ...
    for bid in tasks:
        for t in tasks[bid]:
            project = 'unknown' if t['project'] == '' else t['project']

            if project not in bugs_by_project:
                bugs_by_project[project] = {}

            if bid not in bugs_by_project[project]:
                bugs_by_project[project][bid] = []

            if t['bug_target_name'] not in bugs_by_project[project][bid]:
                bugs_by_project[project][bid].append(t['bug_target_name'])

    report_order = []
    if 'unknown' in bugs_by_project:
        report_order.append('unknown') # We want unknown first
    for t in sorted(bugs_by_project):
        if t != 'unknown':
            report_order.append(t)
%>
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr" lang="en-US">
    ...
    <body class="bugbody">
        ...
        <div class="outermost">
            ...
            <div class="section">
                % for project in report_order:
                    <% bug_count = len(bugs_by_project[project]) %>
            ...
            <div>
                ...
                <table width="100%%" cellspacing="0" cellpadding="0">
                    ...
                    <tbody>
                        ...
                        <tr><td>Series    </td><td>The Ubuntu series that the bug was found in.                         </td></tr>
        ...
        function update_tables() {
            ...
            $.each(jd, function(bid, tasks) {
                $.each(tasks, function(index, task) {
                    ...
                    s = "";
                    if (!fail) {
                        if (oddness[task.task_name]) {
                            s += "<tr class=\"odd\">";
                            oddness[task.task_name] = false;
                        } else {
                            s += "<tr class=\"even\">";
                            oddness[task.task_name] = true;
                        }
