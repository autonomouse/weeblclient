...
def move_report(report, repository):
...
def setup_page(page, since_when, title, team, repository):
    ...
    for line in templatefile:
        if line.strip() == "<!-- *** Title Space *** -->":
            ...
            for task in ubuntu.searchTasks(bug_supervisor=team, created_since=since_when, status='New', tags='iso-testing', tags_combinator='All', order_by='-heat'):
                #FIXME: this should use related tasks or equivalent version.
                tasks = task.bug.bug_tasks
                num_tasks = 0
                for t in task.bug.bug_tasks:
                    if not t.is_complete:
                        num_tasks+=1
                    
                if (num_tasks) == 2:
                    if tasks[0].bug_target_name.split(' ')[0] == tasks[1].bug_target_name.split(' ')[0]:
                        for task in tasks:
                            if 'Oneiric' in task.bug_target_name and task.status != 'New':
                                continue
                ...
                if "(Ubuntu)" in task.bug_target_display_name:
                    try:
                        ...
                        table_row += "<td>%s</td>" % re.sub('\ \(Ubuntu\)','', task.bug_target_display_name.encode('utf-8'))
                        try:
            ...
            datafile.write(line)
    datafile.close()
    move_report(os.path.basename(datafile.name), repository)

home = os.getenv('HOME') + "/scripts/"
template = home + 'general-template.html'
highlist = []
cachedir = os.path.expanduser("/home/bryce/.launchpadlib/cache/")
root = 'production'
...
credfile = os.path.expanduser('/home/bryce/.launchpadlib/%s.cred' % script_name)
...
foundations_team = launchpad.people['foundations-bugs']
server_team = launchpad.people['ubuntu-server']

setup_page('desktop-new-iso-testing.html', '2011-01-01', 'Desktop New bugs marked iso-testing', desktop_team, '/srv/qa.ubuntu.com/reports/ubuntu-desktop/')
setup_page('foundations-new-iso-testing.html', '2011-01-01', 'Foundations New bugs marked iso-testing', foundations_team, '/srv/qa.ubuntu.com/reports/foundations-bugs/')
setup_page('server-new-iso-testing.html', '2011-01-01', 'Server New bugs marked iso-testing', server_team, '/srv/qa.ubuntu.com/reports/ubuntu-server/')
